"""
Macro Economic Agent for AutoInvestor

Provides market regime detection using Federal Reserve Economic Data (FRED).
Analyzes macroeconomic conditions to adjust position sizing and risk exposure.

Key Indicators:
- Yield Curve (T10Y2Y): Recession predictor - inverted curve signals trouble
- VIX (VIXCLS): Fear gauge - high volatility = high risk
- Credit Spreads (BAMLH0A0HYM2): Credit stress indicator
- Federal Funds Rate (DFF): Monetary policy tightness
- Unemployment Rate (UNRATE): Labor market health

Requires: fredapi library and FRED API key (free from https://fred.stlouisfed.org/docs/api/api_key.html)
"""
...
class MacroAgent:
    ...
    VIX_ELEVATED = 20     # Above = concern
    VIX_HIGH = 25         # Above = fear
    VIX_EXTREME = 35      # Above = panic

    CREDIT_SPREAD_NORMAL = 3.5    # Normal high-yield spread
    CREDIT_SPREAD_ELEVATED = 5.0  # Stress emerging
    CREDIT_SPREAD_CRISIS = 7.0    # Credit crunch territory
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        self._cache_duration = timedelta(hours=1)  # Cache data for 1 hour
        ...
        if HAS_FRED and self.api_key:
            try:
                ...
                self.logger.warning(f"Failed to initialize FRED API: {e}")
                self.fred = None
        elif not HAS_FRED:
            self.logger.warning("fredapi not installed. Run: pip install fredapi")
        elif not self.api_key:
            self.logger.warning("No FRED API key provided. Set FRED_API_KEY env var.")
    ...
    def _fetch_series(self, series_id: str, fallback: Optional[float] = None) -> Optional[float]:
        """
    ...
    def get_market_regime(self) -> Dict:
        ...
        # Scoring system
        risk_score = 0  # Higher = more risk, range roughly -2 to +10
        ...
        if credit_spread is not None:
            if credit_spread >= self.CREDIT_SPREAD_CRISIS:
                risk_score += 3
                warnings.append(f"Credit spreads at crisis levels ({credit_spread})")
...
if __name__ == "__main__":
    print("Testing MacroAgent...")
    print()

    agent = MacroAgent()

    if agent.fred is None:
        print("WARNING: FRED API not available")
        print("To use this module:")
        print("  1. pip install fredapi")
        print("  2. Get free API key from: https://fred.stlouisfed.org/docs/api/api_key.html")
        print("  3. Set environment variable: FRED_API_KEY=your_key_here")
        print()
        print("Showing example output structure:")
        print()
        example = {
            "regime": "NEUTRAL",
            "risk_modifier": 0.75,
            ...
            "recommendation": "MODERATE CAUTION - Some warning signs present",
            "indicators": {
                "yield_curve": {"value": 0.25, "interpretation": "FLAT - Economic uncertainty"},
                "vix": {"value": 18.5, "interpretation": "NORMAL - Typical volatility"},
                "credit_spread": {"value": 4.2, "interpretation": "NORMAL - Typical credit conditions"},
                "fed_funds_rate": {"value": 5.25, "interpretation": "VERY RESTRICTIVE - Aggressive tightening"},
                "unemployment": {"value": 3.8, "interpretation": "STRONG - Tight labor market"}
            },
            "warnings": ["Flat yield curve - uncertainty", "Very restrictive Fed policy (5.25%)"],
            "positives": ["VIX normal (18.5)", "Credit conditions stable"]
        }
        import json
        ...
        print("Raw data:")
        import json
        print(json.dumps(agent.get_market_regime(), indent=2, default=str))
