"""
Congressional Trades Aggregate Analysis Module for AutoInvestor

Transforms individual congressional trade disclosures into aggregate trend analysis.
Addresses the 0-45 day disclosure delay limitation by identifying patterns across
multiple Congress members over time.

Key Insight: Individual delayed trades are noise. Patterns across 535 members are signal.
...
def get_all_recent_trades(api_key: Optional[str] = None, limit: int = 100) -> Tuple[List[Dict], Optional[str]]:
    """
    ...
    try:
        ...
        # Parse trades
        trades_list = all_trades_data if isinstance(all_trades_data, list) else all_trades_data.get('trades', [])
        ...
        for trade in trades_list[:limit]:
            ...
            normalized_trades.append({
                ...
                'state': trade.get('state_abbreviation', trade.get('state_name', 'Unknown')),
...
def analyze_ticker_sentiment(trades: List[Dict]) -> Dict:
    ...
    Returns dict with ticker sentiment analysis
    """
...
def analyze_sector_trends(trades: List[Dict]) -> Dict:
    ...
    Uses yfinance to map tickers to sectors, then analyzes sector-level sentiment
    """
    ...
    for ticker in unique_tickers:
        try:
            ...
            # Silently handle invalid tickers
            ticker_sectors[ticker] = 'Unknown'
...
def analyze_party_divergence(trades: List[Dict]) -> Dict:
    """
    Identify tickers where Democrats and Republicans are trading opposite directions

    Interesting for detecting partisan information asymmetries
    """
    ticker_party_stats = defaultdict(lambda: {
    ...
    for ticker, party_data in ticker_party_stats.items():
        ...
        if (dem_net > 0 and rep_net < 0) or (dem_net < 0 and rep_net > 0):
            divergences.append({
                ...
                'democrat_trades': f"{party_data['Democrat']['buys']} buys, {party_data['Democrat']['sells']} sells",
                'republican_trades': f"{party_data['Republican']['buys']} buys, {party_data['Republican']['sells']} sells",
...
def get_aggregate_analysis(api_key: Optional[str] = None) -> Dict:
    ...
    # Date range
    dates = [datetime.strptime(t['transaction_date'], '%Y-%m-%d') for t in trades]
...
def _format_ticker_sentiment(ticker_sentiment: Dict, top_n: int = 15) -> str:
    ...
    lines = []
    lines.append("Ticker  Trades  Buy/Sell  Net  Sentiment         Politicians  Recent(30d)")
    lines.append("-" * 80)

    for i, (ticker, data) in enumerate(list(ticker_sentiment.items())[:top_n], 1):
        recent = f"{data['recent_activity']['buys_30d']}B/{data['recent_activity']['sells_30d']}S"
...
def _format_sector_trends(sector_trends: Dict) -> str:
    ...
    lines = []
    lines.append("Sector                      Trades  Buy/Sell  Net   Politicians  Top Tickers")
    lines.append("-" * 90)
...
def _format_party_divergence(divergences: List[Dict]) -> str:
    ...
    lines = []
    lines.append("Stocks where Democrats and Republicans are trading opposite directions:\n")

    for div in divergences[:10]:
        lines.append(f"  {div['ticker']}:")
        lines.append(f"    Democrats: {div['democrat_sentiment']} ({div['democrat_trades']})")
        lines.append(f"    Republicans: {div['republican_sentiment']} ({div['republican_trades']})")
...
if __name__ == "__main__":
    ...
    print("Fetching and analyzing congressional trading patterns...\n")

    result = get_aggregate_analysis()

    if "error" in result:
        print(f"ERROR: {result['error']}")
    else:
        print(result['summary'])
        print(f"\n\nAnalyzed {result['raw_data']['metadata']['total_trades']} trades")
