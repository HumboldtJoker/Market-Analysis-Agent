"""
News Sentiment Analysis Module for AutoInvestor

Analyzes financial news sentiment using FinBERT (Financial BERT) model from Hugging Face.
Provides sentiment scores and aggregated analysis for investment research.

Data Sources:
- News: Yahoo Finance RSS feeds (free, no API key required)
- Sentiment Model: FinBERT (ProsusAI/finbert) from Hugging Face
"""
...
def get_news_sentiment(ticker: str, days: int = 7) -> Dict:
    ...
    try:
        ...
        for article in news_items[:20]:  # Limit to 20 most recent articles
            ...
            if pub_date_str_raw:
                try:
                    pub_datetime = datetime.fromisoformat(pub_date_str_raw.replace('Z', '+00:00'))
            ...
            # (FinBERT would require transformers library which is large)
...
def analyze_with_finbert(ticker: str) -> Dict:
    ...
    try:
        ...
        for article in news_data['articles']:
            ...
            article['confidence'] = 'high' if confidence > 0.8 else ('medium' if confidence > 0.6 else 'low')
...
if __name__ == "__main__":
    # Test the news sentiment module
    print("Testing News Sentiment Analysis Module\n")

    test_ticker = "AAPL"
    print(f"Analyzing news sentiment for {test_ticker}...\n")

    result = analyze_news_sentiment(test_ticker)
