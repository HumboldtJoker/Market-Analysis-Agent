"""
Portfolio Correlation & Diversification Module for AutoInvestor

Analyzes portfolio diversification through correlation analysis and volatility metrics.
Helps users understand if their holdings are truly diverse or just different flavors
of the same risk.
...
def analyze_portfolio_correlation(tickers: List[str], period: str = "1y") -> Dict:
    """
...
"""
    ...
    return {
        ...
        "verification_links": {
            ...
            "portfolio_tickers": [f"https://finance.yahoo.com/quote/{t}" for t in tickers]
        },
...
def get_portfolio_metrics(tickers: List[str], period: str = "1y") -> Dict:
    """
    ...
    Returns:
        Dict containing correlation matrix, volatility metrics, and diversification scores
    """
    try:
        # Validate tickers
        ...
        for ticker in valid_tickers:
            ...
            std_dev = stock_returns.std() * np.sqrt(252)  # 252 trading days

            # Beta (vs S&P 500)
            # Require minimum 30 trading days for reliable beta calculation
            MIN_BETA_DATA_POINTS = 30
            aligned_returns = pd.concat([stock_returns, market_returns], axis=1, join='inner')
            ...
            mean_return = stock_returns.mean() * 252  # Annualized
            sharpe = mean_return / std_dev if std_dev != 0 else None
        ...
        diversification_score = int((1 - avg_correlation) * 100)
...
def _format_portfolio_overview(stocks: Dict) -> str:
    ...
    lines = []
    lines.append("Stock          Price      Volatility  Beta   Sharpe  Ann. Return")
    lines.append("─" * 70)

    for ticker, metrics in stocks.items():
        price = f"${metrics['current_price']}"
        volatility = f"{metrics['std_dev']*100:.1f}%" if metrics['std_dev'] else "N/A"
        beta = f"{metrics['beta']:.2f}" if metrics['beta'] is not None else "N/A"
        sharpe = f"{metrics['sharpe_ratio']:.2f}" if metrics['sharpe_ratio'] is not None else "N/A"
        ann_return = f"{metrics['annualized_return']:+.1f}%" if metrics['annualized_return'] else "N/A"

        lines.append(f"{ticker:12s} {price:10s} {volatility:11s} {beta:6s} {sharpe:7s} {ann_return:>10s}")

    return '\n'.join(lines)


def _format_diversification_assessment(correlations: Dict, stocks: Dict) -> str:
    ...
    if avg_corr < 0.3:
        level = "EXCELLENT"
        emoji = "[GREEN]"
        explanation = "Your portfolio shows low correlation between holdings. This is ideal for\nreducing concentrated risk."
    elif avg_corr < 0.5:
        level = "GOOD"
        emoji = "[YELLOW]"
        explanation = "Your portfolio has moderate correlation. There's some overlap in risk, but\noverall diversification is reasonable."
    elif avg_corr < 0.7:
        level = "FAIR"
        emoji = "[ORANGE]"
        explanation = "Your portfolio shows notable correlation between holdings. Consider adding\nuncorrelated assets to improve diversification."
    else:
        level = "POOR"
        emoji = "[RED]"
        explanation = "Your portfolio is highly correlated. Holdings tend to move together, which\nreduces diversification benefits during market stress."

    lines = []
    lines.append(f"Diversification Level: {level} {emoji}")
...
def _format_volatility_metrics(stocks: Dict) -> str:
    ...
    for ticker, metrics in stocks.items():
        ...
        lines.append(f"    -> The 'spikiness factor' - how much this stock bounces around")

        if metrics['beta'] is not None:
            lines.append(f"  Beta: {metrics['beta']:.2f}")
            if metrics['beta'] > 1.5:
                lines.append(f"    -> HIGH: Amplifies market moves significantly (wild rides!)")
            elif metrics['beta'] > 1.0:
                lines.append(f"    -> Moderate: Moves more than the market")
            elif metrics['beta'] > 0.5:
                lines.append(f"    -> Low: Moves less than the market (more stable)")
            else:
                lines.append(f"    -> Very Low: Minimal correlation to market movements")

        if metrics['sharpe_ratio'] is not None:
            lines.append(f"  Sharpe Ratio: {metrics['sharpe_ratio']:.2f}")
            if metrics['sharpe_ratio'] > 1.0:
                lines.append(f"    -> GOOD: Decent return for the risk taken")
            elif metrics['sharpe_ratio'] > 0:
                lines.append(f"    -> Fair: Positive return but consider the volatility")
            else:
                lines.append(f"    -> Poor: Negative or minimal risk-adjusted return")
...
def _format_correlation_matrix(correlations: Dict) -> str:
    ...
    lines.append("  1.00 = Perfect positive correlation (move in lockstep)")
    lines.append("  0.00 = No correlation (independent movements)")
    lines.append(" -1.00 = Perfect negative correlation (move in opposite directions)")
    lines.append("  >0.70 = High correlation (similar risk exposure)")

    return '\n'.join(lines)


def _format_risk_clusters(correlations: Dict, threshold: float = 0.7) -> str:
    ...
    for ticker1, ticker2, corr in high_corr_pairs:
        ...
        if corr > 0:
            lines.append(f"    -> These stocks tend to move together (similar risk)")
        else:
            lines.append(f"    -> These stocks tend to move opposite (natural hedge)")

    lines.append("")
    lines.append("Consider: High positive correlation means these stocks may not provide")
    lines.append("true diversification. During market stress, they could decline together.")

    return '\n'.join(lines)


if __name__ == "__main__":
    # Test the portfolio correlation tool
    import sys
    ...
    result = analyze_portfolio_correlation(tickers, period="1y")

    if "error" in result:
        print(f"ERROR: {result['error']}")
    else:
        print(result['summary'])
        print(f"\n\nDiversification Score: {result['raw_data']['diversification_score']}/100")
